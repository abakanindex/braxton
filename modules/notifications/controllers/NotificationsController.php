...
class NotificationsController extends Controller
{
    ...
    public function actionPoll($seen = 0)
    {
        ...
        $reminders = Reminder::findAll(['user_id' => Yii::$app->user->id, 'status' => Reminder::STATUS_ACTIVE]);
        $currentTime = time();
        $minTime = 0;
        $i = 0;

        foreach ($reminders as $reminder) {
            if (!is_null($reminder->notification_time_from) && ($currentTime > $reminder->notification_time_from))
                $tempTime = Reminder::findClosestTime($currentTime, $reminder, Yii::$app->user->id);
            else
                $tempTime = Reminder::findClosestTime($currentTime, $reminder, Yii::$app->user->id);
        ...
        $closestTimeToPoll = $minTime - $currentTime;
        ...
        $results['closestTimeToPoll'] = $closestTimeToPoll;
    ...
    {
        $class = $this->notificationClass;
        $query = $class::find()->where(['user_id' => Yii::$app->user->id]);
