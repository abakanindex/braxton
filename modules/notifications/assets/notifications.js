    ...
    this.poll = function() {
        $.ajax({
            ...
            success: function(data) {
                ...
                opts.closestTimeToPoll = data.closestTimeToPoll * 1000;

                $.each(notices, function (index, object) {
                ...
                // Update all counters
                for (var i = 0; i < self.opts.counters.length; i++) {
                    if ($(self.opts.counters[i]).text() != notices.length) {
                        $(self.opts.counters[i]).text(notices.length);
                    }
                }

                if (opts.closestTimeToPoll > 0) {
                    setTimeout(function() {
                        self.poll(opts);
                    }, opts.closestTimeToPoll);
                }
            },
            dataType: "json",
