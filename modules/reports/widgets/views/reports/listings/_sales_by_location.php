...
<?php
$chartUrl = Url::to(['/reports/report-widget/sales-location-charts', 'id' => $report->url_id]);
...
$('#standart-widget-types-$report->url_id').on('change', function() { 
  if( this.value == $typePieChart )
      getSalesLocationData('pie', '$chartUrl', '$report->url_id');
  else if( this.value == $typeColumnChart )
      getSalesLocationData('bar', '$chartUrl', '$report->url_id');
...
function getSalesLocationData(type, url, reportId) {         
