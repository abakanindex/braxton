...
<?php
$chartUrl = Url::to(['/reports/report-widget/sales-category-charts', 'id' => $report->url_id]);
...
$('#standart-widget-types-$report->url_id').on('change', function() { 
  if( this.value == $typePieChart )
      getSaleCategoryChartData('pie', '$chartUrl', '$report->url_id');
  else if( this.value == $typeColumnChart )
      getSaleCategoryChartData('bar', '$chartUrl', '$report->url_id');
...
function getSaleCategoryChartData(type, url, reportId) {       
