...
<?php
$chartUrl = Url::to(['/reports/report-widget/leads-status-charts', 'id' => $report->url_id]);
...
$('#standart-widget-types-$report->url_id').on('change', function() { 
  if( this.value == $typePieChart )
      getLeadStatusChartData('pie', '$chartUrl', '$report->url_id');
  else if( this.value == $typeColumnChart )
      getLeadStatusChartData('bar', '$chartUrl', '$report->url_id');
...
function getLeadStatusChartData(type, url, reportId) {       
