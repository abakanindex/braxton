...
<?php
$chartUrl = Url::to(['/reports/report-widget/leads-viewing-charts', 'id' => $report->url_id]);
...
$('#standart-widget-types-$report->url_id').on('change', function() { 
  if( this.value == $typePieChart )
      getLeadViewingChartData('pie', '$chartUrl', '$report->url_id');
  else if( this.value == $typeColumnChart )
      getLeadViewingChartData('bar', '$chartUrl', '$report->url_id');
...
function getLeadViewingChartData(type, url, reportId) {     
    $.getJSON(url, function(response) {  
     ...
     $( '#widget-chart-'+reportId ).show();
      ...
      var startCounter = 0;  console.log(response);
      $.each(response, function(key, val) { 
       labels.push( val.agent ); 
