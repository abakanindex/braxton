            ...
            <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-6 with-maxlength">
                        ...
                        <?= $form->field($topModel, 'name')->textInput(['disabled' => $disabledAttribute, 'class' => 'form-control countdown', 'id' => 'rentals-name'])->label(false); ?>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-12">
                        ...
                        <?= $form->field($topModel, 'description')->widget(\mihaildev\ckeditor\CKEditor::class, [
                            ...
                            'options' => ['id' => 'rentals-description']
                        ])->label(false); ?>
                    </div>
                </div>
            </div>
...
<?php
$script = <<<JS
    $("#rentals-name").maxLength({
        maxChars: 60,
        remainHolder: '.chars-remaining'
    });

    $("#rentals-description").maxLength({
        maxChars: 1000,
    ...
    if (typeof CKEDITOR !== 'undefined') {
        CKEDITOR.instances['rentals-description'].on('change', function() {
            $("#rentals-description").val(CKEDITOR.instances['rentals-description'].getData()).trigger('paste');
            //$("#rentals-description").keydown();
        });
    }

    $('body').on('paste', "#rentals-description", function() {
    })
JS;
