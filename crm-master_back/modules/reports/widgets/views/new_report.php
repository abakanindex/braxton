...
<?php
$chartUrl = Url::to(['/reports/main/charts', 'id' => $report->url_id]);
...
$('#standart-widget-types-$report->url_id').on('change', function() { 
  if( this.value == $typePieChart )
      getChartData('pie', '$chartUrl', '$report->url_id');
  else if( this.value == $typeColumnChart )
      getChartData('bar', '$chartUrl', '$report->url_id');
...
function getChartData(type, url, reportId) {       
    $.getJSON(url, function(response) {  
     ...
     $( '#widget-chart-'+reportId ).show();
      ...
      if ( response.data.length != 0 ) {
       ...
       labels.push( val[response.attribute] );
