...
<?php
$chartUrl = Url::to(['/reports/report-widget/sales-status-charts', 'id' => $report->url_id]);
...
$('#standart-widget-types-$report->url_id').on('change', function() { 
  if( this.value == $typePieChart )
      getSaleStatusChartData('pie', '$chartUrl', '$report->url_id');
  else if( this.value == $typeColumnChart )
      getSaleStatusChartData('bar', '$chartUrl', '$report->url_id');
...
function getSaleStatusChartData(type, url, reportId) {       
