...
<?php
$chartUrl = Url::to(['/reports/report-widget/rentals-viewing-charts', 'id' => $report->url_id]);
...
$('#standart-widget-types-$report->url_id').on('change', function() { 
  if( this.value == $typePieChart )
      getSaleViewingChartData('pie', '$chartUrl', '$report->url_id');
  else if( this.value == $typeColumnChart )
      getSaleViewingChartData('bar', '$chartUrl', '$report->url_id');
...
function getSaleViewingChartData(type, url, reportId) {     
