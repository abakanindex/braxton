...
<?php
$chartUrl = Url::to(['/reports/report-widget/rentals-status-charts', 'id' => $report->url_id]);
...
$('#standart-widget-types-$report->url_id').on('change', function() { 
  if( this.value == $typePieChart )
      getRentalsStatusChartData('pie', '$chartUrl', '$report->url_id');
  else if( this.value == $typeColumnChart )
      getRentalsStatusChartData('bar', '$chartUrl', '$report->url_id');
...
function getRentalsStatusChartData(type, url, reportId) {       
