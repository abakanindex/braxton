...
<?php
$chartUrl = Url::to(['/reports/report-widget/rentals-location-charts', 'id' => $report->url_id]);
...
$('#standart-widget-types-$report->url_id').on('change', function() { 
  if( this.value == $typePieChart )
      getRentalsLocationData('pie', '$chartUrl', '$report->url_id');
  else if( this.value == $typeColumnChart )
      getRentalsLocationData('bar', '$chartUrl', '$report->url_id');
...
function getRentalsLocationData(type, url, reportId) {         
