...
<?php
$chartUrl = Url::to(['/reports/report-widget/leads-type-charts', 'id' => $report->url_id]);
...
$('#standart-widget-types-$report->url_id').on('change', function() { 
  if( this.value == $typePieChart )
      getLeadTypeChartData('pie', '$chartUrl', '$report->url_id');
  else if( this.value == $typeColumnChart )
      getLeadTypeChartData('bar', '$chartUrl', '$report->url_id');
...
function getLeadTypeChartData(type, url, reportId) {     
