 ...
 */
class Sale extends \yii\db\ActiveRecord implements iRelationShip, IfirstRecordModel
{
...
//            'development_unit_id' => 'development_unit_id',
     ...
     * @param $companyId
     */
    public static function getSales($companyId) {
    ...
    /*
     * Description: deprecate use getSales
     */
    public static function getAllSales($companyId, $flagAll)
    {
        if ($companyId == 'main' or $companyId == 0) {
            $query = self::find();
        } else {
    ...
    public static function getMaxUpdateDateForPortal($portalId, $company)
    {
        ...
        return $query
            ->select('s.dateupdated')
            ->from(self::tableName() . ' s')
            ->innerJoin(PortalListing::tableName() . ' pL', 'pL.ref = s.ref')
            ->orderBy('s.dateupdated desc')
            ->where([
    ...
    {
        return Sale::find()->where(['ref' => $ref])->with('agent')->with('contact')->with('gallery')->one();
    }
...
//        $statuses[self::STATUS_UNPUBLISHED] = Yii::t('app', 'Unpublished');
        ...
        $statuses[self::STATUS_DRAFT]       = self::STATUS_DRAFT;
        $statuses[self::STATUS_PENDING]     = self::STATUS_PENDING;
        $statuses[self::STATUS_PUBLISHED]   = self::STATUS_PUBLISHED;
        $statuses[self::STATUS_UNPUBLISHED] = self::STATUS_UNPUBLISHED;
